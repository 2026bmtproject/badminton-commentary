...
用法:
    # 1) 輸出一支標註好的影片(最常用)
    python viz_skeleton.py --video test.mp4 --save test_skeleton.mp4 --device cpu --court court.json --shuttle test_ball.csv --hits hit_frames.csv
...
def extract_skeletons(video_path: str, cache_path: str | None = None,
                      get_inferencer=None, device: str = 'cpu'):
    """
...
def run_batch(args):
    ...
    def get_inferencer():
        if _inf['obj'] is None:
            ...
            print(f"[mmpose] 使用裝置: {args.device}")
...
def main():
    ...
    ap.add_argument(
        '--device',
        default='cpu',
        help='MMPose 推論裝置，例如 cpu、cuda、cuda:0；預設 cpu',
    )
    args = ap.parse_args()
